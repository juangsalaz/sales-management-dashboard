...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Form Edit Status Real Sales</h4>
                {{ Form::open(['url' => '/admin/manajemen-marketing/real-sales/status-update/'.$data_real_sales[0]->id,'class'=>'border p-3 edit-form']) }}
                    <div class="form-row">
                        <div class="form-group col-md-4">
                            <label for="status">Ubah Status <span class="text-danger">*</span></label>
                            <select name="status" class="form-control">
                                @if($data_real_sales[0]->status == 'Diterima')
                                    <option value="Diterima" selected>Diterima</option>
                                    <option value="Ditolak">Ditolak</option>
                                    <option value="Pengajuan">Pengajuan</option>
                                @elseif($data_real_sales[0]->status == 'Ditolak')
                                    <option value="Diterima">Diterima</option>
                                    <option value="Ditolak" selected>Ditolak</option>
                                    <option value="Pengajuan">Pengajuan</option>
                                @elseif ($data_real_sales[0]->status == 'Pengajuan')
                                    <option value="Diterima">Diterima</option>
                                    <option value="Ditolak">Ditolak</option>
