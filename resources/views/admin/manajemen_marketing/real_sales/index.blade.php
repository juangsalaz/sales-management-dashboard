    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                    ...
                    <div class="col-md-4" style="margin-bottom: 10px;">
                        @foreach ($data_sub_menu2 as $action)
                            @if ($action->nama =='Tambah')
                                <a href="{{route('realsales-add')}}" class="btn btn-primary" role="button">TAMBAH</a>
                            @endif
                        @endforeach
                        <a href="{{route('realsales-export')}}" class="btn btn-link" role="button">Export to Excel</a>
                    </div>
                    <div class="col-md-8">
                        <form action="{{route('realsales')}}" method="POST" class="form-inline" style="float:right" autocomplete="off">@csrf
                ...
                <form action="{{route('realsales-update-status')}}" method="post">@csrf
                            ...
                            <tbody>
                                @foreach($real_sales as $real_sales)
                                    <tr>
                                        ...
                                        @if ($jabatan_pengguna=="Superuser"||$jabatan_pengguna=="Direktur")
                                            <td>
                                                <input type="checkbox" class="form-control notif-check" name="notif[]" value="{{ $real_sales->id }}">
                                            </td>
                                        @endif
